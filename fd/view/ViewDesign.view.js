...
sap.ui.core.mvc.JSView.extend("fd.view.ViewDesign", {
	...
	/*new sap.m.Input( { 
						value: "{name}",
						editable: false,
						valueState: { path: "name", formatter: function(val) {
							var context = this.getBindingContext();
							if (context) {
								var invalid = context.getProperty("invalid");
								if (invalid) {
									this.setValueStateText(invalid);
									return "Error";
								}
							}
							return "None";
...
template:  new sap.ui.layout.HorizontalLayout({
					content: [
						...
						new sap.ui.commons.TextView( { text: "{name}" }),
					]
	...
	_createAggrTable: function(oControl) {
			...
			var tfValue = new sap.ui.commons.TextField( 
					{ 
					  ...
					  //??now even for the normal binding, it still can add the value for binding path, sorting, 
					  //?? add enhance later such as items="{ 	path :"/Products", 		sorter : { 	path :"Name", 	descending : false 	} }">
					  /*visible: {
						  parts: ['{altTypesStr}',"{multiple}"],
						  formatter: function(alt, mul) {
							  return ( (alt!="") && !mul) ||  mul;
						  }
					  } */
					});
